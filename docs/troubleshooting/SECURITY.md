...
- Les logs, diagnostics, backups automatiques et exports de métadonnées appliquent une redaction automatique aux cookies, tickets, mots de passe et tokens.
...
- L’export distinct de sessions brutes est volontairement en texte clair, exige une confirmation explicite et écrit uniquement dans le dossier local `exports`. Il n’est jamais déclenché par un backup ou un export de métadonnées.
...
- L'API HTTP locale est désactivée par défaut, limitée à `127.0.0.1` et protégée par bearer. Les routes de cookie/CSRF restent interdites tant que la permission indépendante `allow_get_cookie` n’est pas activée.
...
Le monitor ne suit que les processus Roblox connus. La fermeture manuelle exige une confirmation ; les règles mémoire/titre/timeout exigent l’activation globale de fermeture et leur opt-in indépendant, ignorent la fenêtre au premier plan et vérifient processus + fenêtre. Nexus relaie les commandes/scripts demandés uniquement aux clients authentifiés par le jeton éphémère du serveur local ; ses logs sont redacted.
